		    ...
		    cols: [[
		      ...
		      {field:'STATUS', title:'状态', width:185,sort:true, templet:function(d){
		    	  if(window.top.usercode==0){
		    		  ...
		    		  if(d.STATUS==1){
		    			  ...
		    			  for(var item of window.top.arr){
			    			  if(item.UUID=="BEA09414E75C46469619BC717AC8BA34"){//状态
		      ...
		      {fixed: 'right', title:'操作',width:160, templet:function(d){
		    	  	 if(d.STATUS==1){
			    		  ...
			    		  for(var item of window.top.arr){
			    			  if(item.UUID=="AFB6DBE88B1D4244B3E2CE8D5BC7BE59"){//编辑
			    		  ...
			    		  for(var item of window.top.arr){
			    			  ...
			    			  if(item.UUID=="A051DD070B4D4D60B97093019194B361"){//删除
	      ...
	      layer.confirm('确定删除?', function(index){
	        $.ajax({
	    	 ...
	    	 layer.open({
		    	 type:2,
		    	 title:"修改课程",
		    	 content:"course_add.html?fid="+data.FID,
		    	 area:["90%","91%"],
		     })
	    }
	  });
